...
func StyleFor(layerName, class string, zoom int) (LayerStyle, bool) {
	...
	styles := map[string]LayerStyle{
		...
		// Roads — geometry only (no names; those live in transportation_name)
		...
		// Place names (cities, towns, villages, suburbs)
		"place": {
			DrawLabel:  true,
		...
		// Colors match the corresponding transportation geometry layer.
		...
		"transportation_name/secondary": {
			DrawLabel:  true,
			LabelColor: braille.RGBToXterm256(200, 200, 200),
			MinZoom:    12, MaxZoom: 22, Priority: 27,
		},
		// Fallback for any other transportation_name class
		"transportation_name": {
			DrawLabel:  true,
			LabelColor: braille.RGBToXterm256(160, 160, 160),
			MinZoom:    13, MaxZoom: 22, Priority: 25,
		},
	}
